...
`ui/`にHome/Tx/Rx/Frequency/SymbolRate/FEC/Modulation/VideoSource/StreamOutput/Manual/Settings/TestEquipment/AFC(相手局検索)全画面を実装済み。設定は`SettingsStore`でSharedPreferencesへ永続化。
...
`ui/` implements all of the Home/Tx/Rx/Frequency/SymbolRate/FEC/Modulation/VideoSource/StreamOutput/Manual/Settings/TestEquipment/AFC (Find Station) screens. Settings are persisted to SharedPreferences via `SettingsStore`.
