...
// ---------------------------------------------------------------------------
// AFC用: IQストリーミングを開始せず、AD9361の受信LOとRSSI属性だけを操作する小さな
// セッション。送受信パイプラインとは同時に使用しない(Android UI側で開始を抑止する)。
class AfcSession {
public:
...
Java_com_shinjo_shonanandroid_dvbs2_AfcNativeSession_nativeOpen(
        JNIEnv *env, jobject /* thiz */, jstring jUri) {
    const char *uri = env->GetStringUTFChars(jUri, nullptr);
    auto *session = new AfcSession();
...
Java_com_shinjo_shonanandroid_dvbs2_AfcNativeSession_nativeMeasure(
        JNIEnv * /* env */, jobject /* thiz */, jlong handle, jlong frequencyHz) {
    auto *session = reinterpret_cast<AfcSession *>(handle);
    return session ? session->measure(frequencyHz) : NAN;
}

extern "C" JNIEXPORT void JNICALL
Java_com_shinjo_shonanandroid_dvbs2_AfcNativeSession_nativeClose(
        JNIEnv * /* env */, jobject /* thiz */, jlong handle) {
    delete reinterpret_cast<AfcSession *>(handle);
}
