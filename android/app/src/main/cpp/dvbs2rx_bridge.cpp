...
// ★パイロットは常にONで固定する。TX側はPlutoSettingsWriterが常にpilots=Onを
// Plutoへ書き込む(Dvbs2TestRunnerのソフトウェア変調(aff3ct/dvbs2のFramer)も
// パイロットを常に挿入する)。RX側もplsync_ccのPLSフィルタでパイロットONのPLS値だけを
// 許可し、TXと明示的に一致させる。
constexpr bool kPilotsOn = true;

// DVB-S2のMODCOD番号(EN 302 307-1 Table 12)。DVB-S2に存在しない組み合わせは0。
int dvbs2Modcod(dvb_constellation_t constellation, dvb_code_rate_t codeRate) {
    struct Entry { dvb_constellation_t c; dvb_code_rate_t r; int modcod; };
    static const Entry table[] = {
        {MOD_QPSK, C1_4, 1}, {MOD_QPSK, C1_3, 2}, {MOD_QPSK, C2_5, 3}, {MOD_QPSK, C1_2, 4},
        {MOD_QPSK, C3_5, 5}, {MOD_QPSK, C2_3, 6}, {MOD_QPSK, C3_4, 7}, {MOD_QPSK, C4_5, 8},
        {MOD_QPSK, C5_6, 9}, {MOD_QPSK, C8_9, 10}, {MOD_QPSK, C9_10, 11},
        {MOD_8PSK, C3_5, 12}, {MOD_8PSK, C2_3, 13}, {MOD_8PSK, C3_4, 14},
        {MOD_8PSK, C5_6, 15}, {MOD_8PSK, C8_9, 16}, {MOD_8PSK, C9_10, 17},
        {MOD_16APSK, C2_3, 18}, {MOD_16APSK, C3_4, 19}, {MOD_16APSK, C4_5, 20},
        {MOD_16APSK, C5_6, 21}, {MOD_16APSK, C8_9, 22}, {MOD_16APSK, C9_10, 23},
        {MOD_32APSK, C3_4, 24}, {MOD_32APSK, C4_5, 25}, {MOD_32APSK, C5_6, 26},
        {MOD_32APSK, C8_9, 27}, {MOD_32APSK, C9_10, 28},
    };
    for (auto &e : table) if (e.c == constellation && e.r == codeRate) return e.modcod;
    return 0;
}

// PLS値 = (MODCOD << 2) | (short_fecframe << 1) | pilots (gr-dvbs2rxのdvbs2_pls()と同じ)。
// Normal Frame固定(FECFRAME_NORMAL)なのでshort_fecframe=0。
int dvbs2Pls(int modcod, bool pilots) {
    return (modcod << 2) | (pilots ? 1 : 0);
}
...
struct Dvbs2rxSession {
    ...
    bool prepare(const std::string &uri, const std::string &constellationStr,
        ...
        const int modcod = dvbs2Modcod(constellation, codeRate);
        if (modcod == 0) {
            reportError("DVB-S2に存在しない変調方式/符号化率の組み合わせです(" +
                        constellationStr + " " + codeRateStr + ")");
            return false;
        }
        const int targetPls = dvbs2Pls(modcod, kPilotsOn);
        const uint64_t plsFilterLo = targetPls < 64 ? (1ULL << targetPls) : 0;
        const uint64_t plsFilterHi = targetPls >= 64 ? (1ULL << (targetPls - 64)) : 0;
        __android_log_print(ANDROID_LOG_INFO, LOG_TAG, "prepare: modcod=%d pilots=%d pls=%d",
                             modcod, kPilotsOn ? 1 : 0, targetPls);
        ...
        try {
            ...
            // ★PLSフィルタは使用中のMODCOD・Normal Frame・パイロットONに対応する単一PLS値
            // だけを許可する(以前は全PLS許容の0xFFFF...FFFFで、パイロットON/OFFを問わず
            // 受け付けていた)。acm_vcm/multistreamは実機で動作実績のあるtrueのまま据え置く。
            auto plsyncBlock = plsync_cc::make(0, 30, sps, 0, true, true,
                                                plsFilterLo, plsFilterHi);
            plsync = plsyncBlock;
