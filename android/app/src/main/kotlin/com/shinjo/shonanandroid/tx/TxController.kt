...
class TxController(context: Context, lifecycleOwner: LifecycleOwner) {
    ...
    fun start(settings: AppSettings) {
        ...
        if (usingPhoto) {
            ...
            photoSource.start(settings.selectedPhotoUri, OverlaySpec.from(settings), VIDEO_WIDTH, VIDEO_HEIGHT, VIDEO_FPS)
            ...
            // コールサイン・備考・日時はカメラ映像にも重ねる(Shonan_Lite-winと同じ。
            // CameraCaptureはフレームごとに新しい配列を渡すため、そのまま上書きしてよい)。
            val overlay = NV12OverlayBlender(OverlaySpec.from(settings))
            camera.onFrame = { data, frameWidth, frameHeight, presentationTimeUs ->
                overlay.apply(data, frameWidth, frameHeight)
                if (!encoderStarted) {
                    encoderStarted = true
                    videoWidth = frameWidth
                    videoHeight = frameHeight
