...
object SettingsStore {
    ...
    private object Keys {
        ...
        const val TX_DESTINATION_PORT = "txDestinationPort"
        ...
        const val OVERLAY_CALLSIGN_FONT_SIZE = "overlayCallsignFontSize"
        const val OVERLAY_NOTE_FONT_SIZE = "overlayNoteFontSize"
        const val OVERLAY_CALLSIGN_COLOR = "overlayCallsignColor"
        const val OVERLAY_NOTE_COLOR = "overlayNoteColor"
    ...
    fun load(context: Context): AppSettings {
        ...
        return AppSettings(
            ...
            selectedBand = prefs.getString(Keys.BAND, null)
                ...
                ?.takeUnless { it.isLoopback }
                ?: default.selectedBand,
            txDestinationIP = prefs.getString(Keys.TX_DESTINATION_IP, null) ?: default.txDestinationIP,
            // 実機Pluto送信ポートは8282固定運用のため、保存済みの値に関わらず常に既定値を使う。
            txDestinationPort = default.txDestinationPort,
            ...
            overlayCallsignFontSize = prefs.getInt(Keys.OVERLAY_CALLSIGN_FONT_SIZE, default.overlayCallsignFontSize),
            overlayNoteFontSize = prefs.getInt(Keys.OVERLAY_NOTE_FONT_SIZE, default.overlayNoteFontSize),
            overlayCallsignColor = prefs.getString(Keys.OVERLAY_CALLSIGN_COLOR, null) ?: default.overlayCallsignColor,
            overlayNoteColor = prefs.getString(Keys.OVERLAY_NOTE_COLOR, null) ?: default.overlayNoteColor,
    ...
    fun save(context: Context, settings: AppSettings) {
        context.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE).edit().apply {
            ...
            putInt(Keys.TX_DESTINATION_PORT, settings.txDestinationPort)
            ...
            putInt(Keys.OVERLAY_CALLSIGN_FONT_SIZE, settings.overlayCallsignFontSize)
            putInt(Keys.OVERLAY_NOTE_FONT_SIZE, settings.overlayNoteFontSize)
            putString(Keys.OVERLAY_CALLSIGN_COLOR, settings.overlayCallsignColor)
            putString(Keys.OVERLAY_NOTE_COLOR, settings.overlayNoteColor)
