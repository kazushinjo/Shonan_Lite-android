...
internal fun runtimeText(text: String, language: AppLanguage): String {
    ...
    val translated = text
        ...
        .replace("受信中は相手局検索できません。受信を停止してから実行してください。", "Find Station cannot run while RX is active. Stop RX first.")
        .replace("送信中は相手局検索できません。送信を停止してから実行してください。", "Find Station cannot run while TX is active. Stop TX first.")
        .replace("待機中", "Ready")
