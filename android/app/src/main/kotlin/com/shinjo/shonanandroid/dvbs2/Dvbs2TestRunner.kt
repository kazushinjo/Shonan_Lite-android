...
class Dvbs2TestRunner(
        ...
        diagJob = scope.launch {
            try {
                ...
                if (txConnected) {
                    ...
                    // ★1.5秒では最初の確認がウォームアップ中に当たりTXが「異常」になることが
                    // あったため、Shonan_Lite-win(testequipment.pyのWARMUP_MS)と同じ4秒に延ばす。
                    ...
                    for (tick in 1..txSeconds) {
                        ...
                        val healthy = starved < 20.0 && dmaRms > 10.0
                        if (healthy) txHealthySamples++
                        // 異常と判定された場合に原因(枯渇か振幅か)を追えるよう、毎回の値を残す。
                        appendLog(
                            "[Tx] $tick/${txSeconds}秒: starved=${"%.1f".format(starved)}%, " +
                                "rms=${"%.1f".format(dmaRms)} → ${if (healthy) "OK" else "NG"}",
                        )
                    }
                    dummyDataJob.cancel()
                } else {
    ...
    companion object {
        /** TX開始後、健全性の判定を始めるまでの待機時間。Shonan_Lite-winのWARMUP_MSと同じ。 */
        private const val TX_WARMUP_MS = 4_000L
    }
