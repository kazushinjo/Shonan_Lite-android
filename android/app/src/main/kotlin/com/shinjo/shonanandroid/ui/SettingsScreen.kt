    ...
    Scaffold(
        topBar = {
            TopAppBar(
                ...
                navigationIcon = {
                    HomeBackAction({ navController.popBackStack("home", false) }, settings.t("ホームへ戻る", "Home"))
                },
            )
        },
    ) { padding ->
        Column(
            ...
            ExposedDropdownMenuBox(expanded = bandExpanded, onExpandedChange = { bandExpanded = it }) {
                OutlinedTextField(
                    value = settings.selectedBand.displayName,
                    onValueChange = {},
                    readOnly = true,
                    label = { Text(settings.t("バンドプロファイル", "Band Profile")) },
                ...
                DropdownMenu(expanded = bandExpanded, onDismissRequest = { bandExpanded = false }) {
                    BandProfile.entries.filterNot { it.isLoopback }.forEach { band ->
                        androidx.compose.material3.DropdownMenuItem(
                            text = { Text(settings.t(band.displayName, band.displayNameEnglish)) },
                            onClick = {
                                viewModel.updateSettings { it.copy(selectedBand = band) }
                                bandExpanded = false
                            },
                        )
                    }
            ...
            Text(settings.t("バンド情報", "Band Information"), style = MaterialTheme.typography.titleMedium)
            Text(
                settings.t(
                    "推奨Wi-Fi帯: ${settings.selectedBand.recommendedWiFiBand.label}",
                    "Recommended Wi-Fi band: ${settings.selectedBand.recommendedWiFiBand.labelEnglish}",
                ),
            )
            Text(
                settings.t(settings.selectedBand.wiFiBandReason, settings.selectedBand.wiFiBandReasonEnglish),
                style = MaterialTheme.typography.bodySmall,
            )
            if (!settings.selectedBand.isLoopback) {
                Text(settings.t("実効シンボルレート: ${settings.effectiveOperationalSymbolRateMsps} Msym/s / 映像: ${settings.effectiveVideoBitrateBps / 1_000} kbps", "Effective symbol rate: ${settings.effectiveOperationalSymbolRateMsps} Msym/s / Video: ${settings.effectiveVideoBitrateBps / 1_000} kbps"))
            }
            ...
            if (settings.useOnDeviceGRDVBS2Rx) {
                Text(settings.t("ロールオフ: ${settings.dvbs2Rolloff}(固定)", "Roll-off: ${settings.dvbs2Rolloff} (fixed)"))
                Text(
                    settings.t(
        ...
        AlertDialog(
            ...
            text = {
                Column {
                    Text(
                        settings.t(
                            "オンデバイス復調をONにすると、送信中でも受信を停止せずに済むようになり、送受信を同時に行えます。",
                            "Enabling on-device demodulation allows transmit and receive to run at the same time, without stopping reception during transmission.",
                        ),
                    )
                    Text(
                        settings.t(
