...
object NetworkDefaults {
    ...
    /** ループ試験モード専用のUDP TS送出先ポート既定値(実機Pluto送信には使わない)。 */
...
data class AppSettings(
    ...
    var txDestinationPort: Int = NetworkDefaults.PLUTO_UDP_TS_PORT,
    ...
    /** カメラ・写真の映像へ重ねるコールサイン(テストパターンには重ねない)。
     *  ★保存キー互換のため名前はphoto*のまま。 */
    var photoCallsign: String = "",
    /** カメラ・写真の映像へ重ねる備考。 */
    var photoNote: String = "",
    /** コールサインの文字サイズ(px、1920x1080の送信映像上での大きさ。Win版と同じ既定値)。 */
    var overlayCallsignFontSize: Int = 68,
    /** 備考の文字サイズ(px、1920x1080上)。 */
    var overlayNoteFontSize: Int = 24,
    /** コールサインの文字色("#RRGGBB")。 */
    var overlayCallsignColor: String = "#FFFFFF",
    /** 備考の文字色("#RRGGBB")。 */
    var overlayNoteColor: String = "#FFFFFF",
    ...
    /** 実際に使うUDP送出先を解決する -- 実機Pluto送信時は`txDestinationPort`(UI設定、
     *  ループ試験専用)を無視し、F5OEO標準ファームウェアのudpts.sh固定ポートへ送る
     *  (mac版`TxSessionController`と同じ区別)。 */
    fun effectiveTxDestination(): Pair<String, Int> {
        val port = if (selectedBand.isLoopback) txDestinationPort else NetworkDefaults.PLUTO_UDP_TS_PORT
        return txDestinationIP to port
    }
